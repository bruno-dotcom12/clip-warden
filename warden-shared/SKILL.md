...
| `echo "<key>" \| warden post setkey` | how the PERSON'S OWN key gets in. It reads the key from standard input and never from an argument -- an argument sits in `ps`, in the shell history and in the runtime's command log, and all three are read by people who should not read the key to someone's channel -- and writes it 0600 into this machine's volume. It prints the path and never the key, not a prefix of it and not its length. It exists because on Plow's cloud there is no `.env` and no `compose.yml`: without it the only key available is the agent owner's, or none |
| `warden post connect [<profile>]` | ONE address to hand them, and the only step that does not fit in the chat: they pick the channel on Google's own screen and press Allow, and their password is typed there and nowhere else. Run it again after -- it prints `already connected` when it worked. It also creates this machine's own profile when there is none: the name is `clip-warden-<8 hex>`, decided the first time and kept in the volume. It used to be `clip-warden` for every install, which with one shared key would have put every machine on the same channel |
| `warden post status` | is a publishing intermediary configured on this install, and which account is connected. It also prints WHERE the key came from and where the profile name came from -- the environment or this machine's own file, never the value of the key. Read the origin when the channel is not the one they expected: a key from the environment is the agent owner's, and then the profile here has to be this machine's alone. Exit 1 here means no key, and the persona's "Publishing" section says what to offer |
| `warden post status <request_id>` | that ONE upload instead of the account: where it is in the intermediary's queue, and the address when it lands. It was dead code until 16/09 -- the account branch came first and always returned, so the id was read and ignored in silence, and the agent that ran it looking for the video's address got a list of the account instead and stayed without the one thing it had promised to send back |
| `warden post youtube <clip> --title "..." [--description "..."]` | **the road that actually publishes.** Goes through an intermediary whose app Google already audited, so the video comes out PUBLIC on their channel. Measured with real uploads on 15 and 16/09. The title is REFUSED above 100 characters rather than truncated: a truncated title goes to the owner's channel as a sentence nobody wrote. Exits 1 saying it is switched off when this install has no key -- neither in `WARDEN_POST_API_KEY` nor in this machine's own file |
| `warden post tiktok <clip> --title "<caption>" [--draft]` | the same endpoint, TikTok, and the caption is `--title`. **No upload from this project has ever gone through it**: the field names come from the provider's official OpenAPI spec, read 16/09, and nothing else is behind them -- report what the API returns and claim no more than that. `--draft` sends it to TikTok's drafts, and in draft mode TikTok IGNORES the caption and the privacy the API sent, so the caption is written by them, in the app. TikTok has no `unlisted` and the command refuses it instead of choosing a meaning for it; the caption is refused above 2200 characters, not cut. The provider's documentation requires a paid plan there |
| `warden post instagram <clip> --title "<caption>" [--stories]` | the same again, Instagram: a Reel by default, a Story with `--stories`. Also never measured with a real upload -- the field names are the official spec's and that is all. Instagram has no per-post privacy at all, since the profile is public or private whole, so anything but `--privacy public` is refused instead of being sent as if it had been obeyed; the caption limit is the same 2200, refused and not cut. The provider's documentation requires a Business or Creator account |
| `warden post <net> <clip> --title "..." --also tiktok,instagram` | adds networks to the SAME upload: the file goes up once and the intermediary fans it out. What comes back is one address per network, each one as the API returned it -- no address is assembled here out of an id |
